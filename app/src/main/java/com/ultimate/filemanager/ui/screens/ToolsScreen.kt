...
fun ToolsScreen(
    ...
    val trashItems = remember { TrashStore.listItems(context) }
    val trashDescription = if (trashItems.isEmpty()) {
        "Empty — deleted files show up here before they're gone for good"
    } else {
        "${trashItems.size} item${if (trashItems.size == 1) "" else "s"} · " +
            FileOperations.formatSize(trashItems.sumOf { it.size })
    }
    ...
    val tools = listOf(
        ...
        Tool("Trash", trashDescription, Icons.Outlined.Delete, CategoryColors.Other, onOpenTrash),
