...
fun isInAppViewable(entry: FileEntry): Boolean {
    ...
    if (isTextLike(entry)) return true
...
fun FileViewerHost(
    ...
    // Text/code files manage their own top bar (they need a Save action),
    // so they get their own full Scaffold instead of the shared one below.
    if (isTextLike(entry)) {
        TextViewerScreen(
            entry = entry,
            onClose = onClose,
